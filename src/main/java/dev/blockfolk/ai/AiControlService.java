...
public final class AiControlService {
    ...
    /**
     * Sends one request for all chat-enabled NPCs near a player, ordered closest
     * first.
     */
    public void invokeChatGroup(String eventDetail, List<NpcInstance> candidates, Player player,
        ...
        List<GroupParticipant> eligibleParticipants = candidates.stream()
                ...
                .limit(MAX_CHAT_GROUP_SIZE).toList();
        ...
        if (groupInFlight.putIfAbsent(groupKey, groupSequence) != null) {
            pendingGroups.put(groupKey,
                    new PendingGroupInvocation(eventDetail, List.copyOf(candidates), player, resultHandler));
            schedulePendingGroup(groupKey, cooldownMillis);
            return;
        }

        // An NPC may already be handling a behaviour-triggered AI event. Do not let
        // that
        // participant stall an established conversation; it can join a later chat
        // request.
        List<GroupParticipant> participants = eligibleParticipants.stream()
                .filter(participant -> !inFlight.contains(participant.instance().getId())).toList();
        ...
        long now = System.currentTimeMillis();
        long previous = participants.stream()
                .mapToLong(participant -> lastInvocation.getOrDefault(participant.instance().getId(), 0L)).max()
                .orElse(0L);
        ...
        List<GroupParticipant> claimedParticipants = participants.stream()
                .filter(participant -> inFlight.add(participant.instance().getId())).toList();
        ...
        try {
            for (int index = 0; index < participants.size(); index++) {
                ...
                context.append("\n=== ").append(alias).append(": ").append(participant.definition().getDisplayName())
                        .append(index == 0 ? " (closest; default speaker)" : "").append(" ===\n")
        ...
        try {
            client.complete(system.toString(), context.toString()).whenComplete((content, error) -> {
                requestParticipants.forEach(participant -> {
                    ...
                    if (generations.getOrDefault(instanceId, 0L).equals(requestGenerations.get(instanceId))) {
                ...
                if (error != null) {
                    ...
                    if (plugin.isEnabled()) {
                        Bukkit.getScheduler().runTask(plugin, () -> applyGroupDecisions(aliases, decisions,
                                requestGenerations, targetsByInstance, player, resultHandler));
                    }
                }
                if (plugin.isEnabled()) {
                    Bukkit.getScheduler().runTask(plugin, () -> {
    ...
    private void applyGroupDecisions(Map<String, GroupParticipant> aliases, Map<String, AiDecision> decisions,
        ...
        for (Map.Entry<String, AiDecision> response : decisions.entrySet()) {
            ...
            for (AiDecision.Action action : response.getValue().actions()) {
                ...
                validParticipants.values().forEach(listener -> memory.rememberMessage(listener.instance().getId(),
                        player.getUniqueId(), listener.settings().sharedConversation(), line));
            }
        }
        ...
        validParticipants.forEach((alias, participant) -> {
            ...
            try {
                resultHandler.accept(participant.instance(),
                        new AiDecisionResult(decision, targetsByInstance.get(participant.instance().getId())));
            } catch (RuntimeException error) {
    ...
    private void schedulePendingGroup(UUID groupKey, long delayMillis) {
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            ...
            List<NpcInstance> nearbyCandidates = invocation.candidates().stream()
                    .filter(candidate -> instances.findById(candidate.getId()).isPresent()).toList();
    ...
    private void appendNearby(StringBuilder out, NpcInstance instance, Entity actor, AiControlSettings settings,
        ...
        for (int index = 0; index < nearbyNpcs.size(); index++) {
            ...
            definitions.find(other.getDefinitionKey()).ifPresent(definition -> out.append("- nearby_npc_")
                    .append(targetIndex).append(": ").append(definition.getDisplayName()).append(", ")
    ...
    private record PendingGroupInvocation(String eventDetail, List<NpcInstance> candidates, Player player,
            BiConsumer<NpcInstance, AiDecisionResult> resultHandler) {
